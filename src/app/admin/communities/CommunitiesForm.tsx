...
type Props = {
  creators: Creator[];
  initial?: { creatorId: string; name: string; slug: string; description?: string; price: number; platform: string; status: string };
  id?: string;
};

export function CommunitiesForm({ creators, initial, id }: Props) {
  ...
  const [platform, setPlatform] = useState(initial?.platform ?? "DISCORD");
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    const url = id ? `/api/admin/communities/${id}` : "/api/admin/communities";
    const res = await fetch(url, {
      method: id ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ creatorId, name, slug, description: description || undefined, price, platform, status }),
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 max-w-md space-y-4">
      ...
      <div><label className="block text-sm font-medium text-slate-700">Platform</label>
        <input type="text" value={platform} onChange={(e) => setPlatform(e.target.value)} placeholder="DISCORD, CUSTOM, etc." className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></div>
